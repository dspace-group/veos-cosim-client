...
- [Step 5: Sending Data](#step-5-sending-data)
  ...
  - [6 Bus log files](#6-bus-log-files)
  - [7 Cleanup](#7-cleanup)
...
You must have run [Step 4: Using Data Callback Functions to Get Information on CAN Bus Messages](step4-callback.md).
...
The function then calls the [DsVeosCoSim_TransmitCanMessage](../api-reference/functions/DsVeosCoSim_TransmitCanMessage.md) function, which transmits the message to the `Controller` VPU via the CoSim server.
...
The `BusLog.pcapng` file contains entries for the bus messages transmitted and received by the CoSim server.
Use Wireshark or a similar tool to investigate the contents of the log file.
...
Refer to [Step 6: Receiving Data](step6-receive.md).
