...
void RunTest(benchmark::State &state, ShmPipeClient &client1,
             ShmPipeClient &client2) {
  ...
  bool stopThread{};
  std::thread thread(CounterPart, std::ref(client1), std::ref(stopThread),
                     size);
